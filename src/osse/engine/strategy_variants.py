...
class StrategyVariantSelector:
    ...
    def select_variants(
        ...
        rules_cfg = self.config.get("dex_vp_strategy_rules", {})
        conf_thresh = rules_cfg.get("confluence_thresholds", {})
        t1_thresh = float(conf_thresh.get("tier1", 80))
        t2_thresh = float(conf_thresh.get("tier2", 60))
        t3_thresh = float(conf_thresh.get("tier3", 40))
        ...
        delta_flip = dex_data.get("delta_flip", spot_price)
        ...
        # ---------------------------------------------------------
        # Variant 1: DEX-VP Confluence Strangle (Non-directional)
        # ---------------------------------------------------------
        if cs >= 80 and val <= spot_price <= vah and vix < 20.0 and (0.8 <= pcr_oi <= 1.2):
            call_strike = self._round_to_strike(max(vah, call_wall), round_up=True)
            put_strike = self._round_to_strike(min(val, put_support), round_up=False)
        ...
        # ---------------------------------------------------------
        # Variant 2: DEX Breakout Rejection Call Credit Spread (Mild Bearish)
        # ---------------------------------------------------------
        if cs >= 60 and (spot_price >= (call_wall * 0.995)) and vix >= 14.0:
            sell_strike = self._round_to_strike(call_wall, round_up=True)
            width_offset = (vah - poc) * 1.5 if (vah > poc) else self.step_size * 2
            buy_strike = self._round_to_strike(sell_strike + width_offset, round_up=True)
            ...
            variants.append({
                ...
                "breakeven": sell_strike, # plus credit
                "entry_conditions": [
        ...
        # ---------------------------------------------------------
        # Variant 3: VP-VAL Bounce Put Credit Spread (Mild Bullish)
        # ---------------------------------------------------------
        if cs >= 60 and (spot_price <= (put_support * 1.005)) and pcr_oi <= 1.0:
            sell_strike = self._round_to_strike(put_support, round_up=False)
            width_offset = (poc - val) * 1.5 if (poc > val) else self.step_size * 2
            buy_strike = self._round_to_strike(sell_strike - width_offset, round_up=False)
            ...
            variants.append({
                ...
                "expiry_preference": "Weekly (5-7 DTE)",
                "recommended_risk_pct": 1.5,
                "breakeven": sell_strike, # minus credit
                "entry_conditions": [
        ...
        # ---------------------------------------------------------
        # Variant 4: DEX-Range Iron Condor (Non-directional)
        # ---------------------------------------------------------
        if cs >= 80 and va_range_pct >= 0.02 and 15.0 <= vix <= 22.0:
            wing_width = self.step_size * 2 if self.symbol == "NIFTY" else self.step_size * 3
            sell_call = self._round_to_strike(vah, round_up=True)
            buy_call = self._round_to_strike(sell_call + wing_width, round_up=True)
            sell_put = self._round_to_strike(val, round_up=False)
            buy_put = self._round_to_strike(sell_put - wing_width, round_up=False)
        ...
        # ---------------------------------------------------------
        # Variant 5: LVN Momentum Ratio Spread (Directional Momentum)
        # ---------------------------------------------------------
        if cs >= 60 and len(lvn_array) > 0 and vix < 18.0:
            # Determine direction relative to delta flip
            if spot_price >= delta_flip:
                # Bullish Call Ratio Spread
                long_strike = self._round_to_strike(spot_price, round_up=True)
                short_strike = self._round_to_strike(max(call_wall, long_strike + self.step_size * 2), round_up=True)
                variants.append({
                    ...
                    "entry_conditions": [
                        f"LVN air pocket detected; Spot ({spot_price:.2f}) above Delta Flip ({delta_flip:.2f})",
                ...
                long_strike = self._round_to_strike(spot_price, round_up=False)
                short_strike = self._round_to_strike(min(put_support, long_strike - self.step_size * 2), round_up=False)
                variants.append({
                    "variant_id": "VARIANT_5",
                    ...
                    "recommended_risk_pct": 1.0,
                    "entry_conditions": [
                        f"LVN air pocket detected; Spot ({spot_price:.2f}) below Delta Flip ({delta_flip:.2f})",
                        f"India VIX ({vix:.1f}) < 18.0"
                    ],
                    "exit_conditions": [
