...
def analyze_5m_intraday_data():
    print("==========================================================")
    print("  AUTHENTIC 5-MINUTE INTRADAY DATA AUDIT (LAST 60 DAYS)   ")
    print("==========================================================")
    ...
    print(f"Loaded {len(unique_dates)} 5-minute intraday sessions from {unique_dates[0]} to {unique_dates[-1]}.\n")
    ...
    for i, date_str in enumerate(unique_dates):
        ...
        if decision['decision'] in ['TRADE', 'REDUCED SIZE'] and triggered:
            if is_wick_exhausted:
                pnl = 0.0
                status = "FILTERED_WICK"
            elif trailing_stop_triggered:
                pnl = 1950.0
                status = "WIN_TRAILING_LOCK"
            elif orb_opposite_triggered:
                pnl = -975.0
                status = "LOSS_ORB_OPPOSITE_CUT"
            else:
                win = (breakout_dir == "UP" and exit_price >= orb_high) or (breakout_dir == "DOWN" and exit_price <= orb_low)
                pnl = net_credit_inr if win else -max_loss_inr
                status = "WIN_CREDIT" if win else "LOSS_FULL"
    ...
    trades = df_rec[df_rec['status'] != 'NO_TRADE']
    wins = trades[trades['pnl'] > 0]
    
    print("------------------------------------------------------------------")
    print(f"Total 5m Sessions Analyzed   : {len(df_rec)}")
    print(f"Total Trades Taken           : {len(trades)}")
    print(f"Total Wins                   : {len(wins)}")
    print(f"Authentic 5m Win Rate %      : {(len(wins)/len(trades)*100.0) if len(trades)>0 else 0:.2f}%")
    print(f"Authentic 5m Net PnL (INR)   : INR +{trades['pnl'].sum():,.2f}")
    print("------------------------------------------------------------------\n")

if __name__ == "__main__":
    analyze_5m_intraday_data()
